			...
			<div className="sticky top-0 z-10 border-b border-white/5 px-4 py-2.5 bg-[#1a1a1a] backdrop-blur">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-3 flex-1 min-w-0">
						<h3 className="text-xs font-medium text-zinc-200 truncate">{file.fileName}</h3>
						<span className="text-[10px] text-zinc-600 truncate">{file.filePath}</span>
					</div>
					<div className="flex items-center gap-3 shrink-0">
						{file.oldPath && file.status === 'renamed' && (
							<div className="text-[10px] text-zinc-500">
								from <span className="font-medium text-zinc-400">{file.oldPath}</span>
							</div>
						)}
						<span
